<template>
  <div>
    ...
    <div class="modal" :class="{ open: modalOpen }">
      <div class="carousel-container">
        ...
        <div class="modal-content">
          ...
          <img v-if="activePhoto.src" :src="activePhoto.src" :alt="categoryNames[activePhoto.category] || ''" class="modal-image">
          
          <!-- Bouton de téléchargement -->
          <button v-if="activePhoto.src" 
                 @click="downloadImage"
    ...
    <div class="download-notification" :class="{ 'active': showDownloadNotification }">
...
export default {
  ...
  methods: {
    ...
    async downloadImage() {
        if (!this.activePhoto || !this.activePhoto.src) return;

        try {
          const response = await fetch(this.activePhoto.src);
          if (!response.ok) {
          ...
          const category = this.categoryNames[this.activePhoto.category] || this.activePhoto.category || 'photo';
          const timestamp = new Date().getTime();
          const filename = `Photo-${category}-${timestamp}.jpg`;
          ...
          // Afficher la notification et la cacher après 3 secondes
          ...
          alert("Une erreur s'est produite lors du téléchargement de l'image."); // Affiche une alerte à l'utilisateur
        }
      }
  },
  watch: {
